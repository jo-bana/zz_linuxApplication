...
char week[]={"Sun\tMon\tTus\tWed\tThu\tFri\tSat\n"};//制表位\t
...
{
	int dates = ((year-1900)*365)+(year-1901)/4-(year-1901)/100+(year-1601)/400;
...
void drawCalendar(int year)
{
	int i,j,k,m,n,weekday,flag=0;
	weekday=getDayFromDate(year,1,1);
	char week[]={"Sun\tMon\tTus\tWed\tThu\tFri\tSat\t Sun\tMon\tTus\tWed\tThu\tFri\tSat\n"};//制表位\t
	for(i=0,m=6;i<6,m<12;i++,m++)
	{
		printf("--------------------  %s  -------------------- \t--------------------  %s  --------------------\n",monthName[i],monthName[m]);
		printf("%s",week);
		for(j=1,n=1,k=0,flag=0;j<=month_day[i],n<month_day[m];k++,j++,n++)
		{
			if(0==flag&&weekday>k)
			{
				printf("\t");
